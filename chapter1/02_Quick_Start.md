...
业务系统中，Domain可以接口与实现使用相同的类，简化开发的工作量，在实现类中通过 annotation 加上数据库表和字段的定义。 Domain 接口类放在 macula-xxx-repository 资源包里（如macula-samples-repository），对应的 package 是类似这样：
...
3. 代码中的 #createApplication() 方法用于初始化一个 DemoApplication，这个将在新增应用信息时使用，
