...
该教程是假设开发人员已经获取到 Macula 平台的代码，并且配置好了 Eclipse 开发环境。下面我们以一个示例一步步详细解释了整个开发过程。我们要实现一个“应用管理”的功能，用于在我们的系统中保存应用信息。我们需要提供新增、编辑、删除应用的功能，以及一个列表功能来显示但前所有的应用信息。

### Domain 和 Repository 层

首先，我们需要一个Domain类用于保存我们的应用信息，然后还需要相对应的Repository类来对应用信息进行增删改查等操作。Macula 平台的 Repository 层基于 Spring-Data-JPA（hibernate） 做了一些封装，功能很全面，只要在指定 package 下定义好 domain model 和 Repository 接口，就可以实现很完善的数据存储功能。
...
下面我们来看Service层的实现。Service 类放在 macula-xxx-service 资源包里（如macula-samples-service）。Service 类一般是先定义 Service 接口类，然后定义 Service 实现类，注意要在 Service 实现类声明上面加上 annotation @Service，我们使用对于涉及到数据库修改的实现，需要加上@Transactional。示例如下：
 ...
 */
package org.macula.samples.macula_samples.demo.service.impl;
...
import org.macula.samples.macula_samples.demo.service.DemoApplicationService;
import org.macula.samples.macula_samples.domain.DemoApplication;
import org.macula.samples.macula_samples.repository.DemoApplicationRepository;
